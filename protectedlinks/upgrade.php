...
upgrade_v_1_2_5($conn);

function upgrade_v_1_2_5($conn){

	$sql = "SELECT COUNT(*) 
                    FROM INFORMATION_SCHEMA.COLUMNS
                    WHERE   TABLE_SCHEMA = '".DBNAME."' AND
							TABLE_NAME = 'settings' AND 
                            COLUMN_NAME = 'usehtaccess_f'";
 $query = mysqli_query($conn,$sql) or die(mysqli_error($conn));
$result = mysqli_fetch_array($query); //print_r($result);
if($result['COUNT(*)']<1){
$sql1 = "AlTER TABLE settings ADD COLUMN usehtaccess_f boolean NOT NULL default false ";
